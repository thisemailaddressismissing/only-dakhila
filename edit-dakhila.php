...
// Get Dakhila ID from GET parameter
$id = (int) ($_GET['id'] ?? 0);
if ($id <= 0) {
...
$stmt = $db->prepare("SELECT * FROM dakhila WHERE id = :id AND user_id = :uid");
$stmt->execute([':id' => $id, ':uid' => $user['id']]);
$record = $stmt->fetch();

if (!$record) {
    die("❌ ত্রুটি: নির্দিষ্ট দাখিলা খুঁজে পাওয়া যায়নি অথবা এটি সম্পাদনা করার আপনার অনুমতি নেই।");
}
...
$existingOwners = $ownerStmt->fetchAll();

// Fetch existing dags
$dagStmt = $db->prepare("SELECT * FROM dakhila_dags WHERE dakhila_id = :id ORDER BY sort_order");
$dagStmt->execute([':id' => $id]);
$existingDags = $dagStmt->fetchAll();
...
$formData = [
    ...
    'mouja_jl'             => $record['mouja_jl'],
    'khatian_no'           => $record['khatian_no'],
    'payment_year_en'      => $record['payment_year_en'],
    'issue_date'           => $record['issue_date'],
    'day'                  => (string) $record['payment_day'],
    'month'                => (string) $record['payment_month'],
    'year'                 => (string) $record['payment_year'],
    'three_years_plus_due' => (string) $record['three_years_plus_due'],
    'last_three_years_due' => (string) $record['last_three_years_due'],
    'due_interest'         => (string) $record['due_interest'],
    'current_demand'       => (string) $record['current_demand'],
    'total_demand'         => (string) $record['total_demand'],
    'total_collection'     => (string) $record['total_collection'],
    'total_due'            => (string) $record['total_due'],
    'comments'             => $record['comments'],
    'total_in_words'       => $record['total_in_words'],
    'owners'               => array_map(fn($o) => ['name' => $o['name'], 'share' => (string)$o['share']], $existingOwners),
    'dags'                 => array_map(fn($d) => ['dag' => $d['dag_no'], 'type' => $d['type'], 'amount' => (string)$d['amount']], $existingDags),
];
    ...
    if (empty($errors)) {
        try {
            ...
            $sql = "UPDATE dakhila SET
                ...
                payment_year_bn      = :payment_year_bn,
                payment_year_en      = :payment_year_en,
                issue_date           = :issue_date,
                payment_day          = :payment_day,
                payment_month        = :payment_month,
                payment_year         = :payment_year,
                three_years_plus_due = :three_years_plus_due,
                last_three_years_due = :last_three_years_due,
            ...
            // Replace owners
            $db->prepare("DELETE FROM dakhila_owners WHERE dakhila_id = :id")->execute([':id' => $id]);
            ...
            // Replace dags
            $db->prepare("DELETE FROM dakhila_dags WHERE dakhila_id = :id")->execute([':id' => $id]);
            if (!empty($_POST['dags'])) {
            ...
            $_SESSION['flash_success'] = "✅ দাখিলা নম্বর #" . h($_POST['registry_no']) . " সফলভাবে আপডেট করা হয়েছে!";
...
// Helper getter function for values
function val($key, $default = '') {
    global $formData;
    return h($formData[$key] ?? $default);
}

function isSelectedVal($key, $expected) {
    global $formData;
    return ((string)($formData[$key] ?? '') === (string)$expected) ? 'selected' : '';
}
?>
<!DOCTYPE html>
<html lang="bn">
<head>
    ...
    <title>দাখিলা সংশোধন — <?= SITE_NAME ?></title>
...
<div class="app-container">
    ...
    <header class="app-navbar">
        ...
        <div class="nav-actions">
            ...
            <a href="view-dakhila?id=<?= $id ?>" target="_blank" class="btn-app btn-app-primary">📄 প্রিন্ট কপি</a>
    ...
    <div class="form-card">
        <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
            <div>
                <h4 class="fw-bold text-dark mb-1">✏️ দাখিলা তথ্য সংশোধন (রেজিঃ #<?= val('registry_no') ?>)</h4>
                <p class="text-muted small mb-0">প্রয়োজনীয় তথ্য সংশোধন শেষে "তথ্য আপডেট করুন" বাটনে ক্লিক করুন।</p>
            </div>
            <a href="index" class="btn-app btn-app-secondary">← ফিরে যান</a>
        </div>
            ...
            <div style="display:flex; flex-direction:column; gap:1.25rem;">
                ...
                <div class="form-grid-5">
                    <div>
                        ...
                        <input type="text" name="registry_no" value="<?= val('registry_no') ?>" readonly class="form-control-custom" style="background-color: #f1f5f9; color: #475569; cursor: not-allowed; font-weight: 600;">
                    </div>
                    <div>
                        <label class="form-label-custom">চালান নম্বর (ঐচ্ছিক)</label>
                        <input type="text" name="challan_no" value="<?= val('challan_no') ?>" class="form-control-custom">
                    </div>
                    <div class="col-span-2">
                        <label class="form-label-custom">সিটি কর্পোরেশন / পৌরসভা / ইউনিয়ন ভূমি অফিসের নাম *</label>
                        <input type="text" name="office_name" value="<?= val('office_name') ?>" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">উপজেলা / থানা *</label>
                        <input type="text" name="upazila" value="<?= val('upazila') ?>" required class="form-control-custom">
                    </div>
                </div>
                ...
                <div class="form-grid-5">
                    <div>
                        ...
                        <input type="text" name="district" value="<?= val('district') ?>" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">হোল্ডিং নম্বর *</label>
                        <input type="text" name="holding_no" value="<?= val('holding_no') ?>" required class="form-control-custom">
                    </div>
                    <div class="col-span-3">
                        <label class="form-label-custom">মৌজা ও জে. এল. নম্বর *</label>
                        <input type="text" name="mouja_jl" value="<?= val('mouja_jl') ?>" required class="form-control-custom">
                    </div>
                </div>
                ...
                <div class="form-grid-5">
                    <div>
                        ...
                        <input type="text" name="khatian_no" value="<?= val('khatian_no') ?>" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">অর্থবছর *</label>
                        <input type="text" name="payment_year_en" id="payment_year_en" value="<?= val('payment_year_en') ?>" required class="form-control-custom" placeholder="2025-2026">
                    </div>
                    <div>
                        <label class="form-label-custom">দাখিলা ইস্যুর তারিখ *</label>
                        <input type="date" name="issue_date" value="<?= val('issue_date') ?>" required class="form-control-custom">
                    </div>
                    <div class="col-span-2">
                        ...
                        <div class="form-grid-3" style="gap:0.5rem;">
                            <div>
                                <select name="day" required class="form-select">
                                    ...
                                    <?php for ($i = 1; $i <= 31; $i++): ?>
                                        <option value="<?= $i ?>" <?= isSelectedVal('day', $i) ?>><?= $i ?></option>
                                    <?php endfor; ?>
                                </select>
                            </div>
                            <div>
                                <select name="month" required class="form-select">
                                    ...
                                    foreach ($months as $k => $v): ?>
                                        <option value="<?= $k ?>" <?= isSelectedVal('month', $k) ?>><?= $v ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div>
                                <select name="year" id="year" required class="form-select">
                                    ...
                                    <?php for ($i = 1400; $i <= 1500; $i++): ?>
                                        <option value="<?= $i ?>" <?= isSelectedVal('year', $i) ?>><?= $i ?></option>
                                    <?php endfor; ?>
                                </select>
                            </div>
                        </div>
                ...
                <div class="form-grid-4">
                    <div>
                        ...
                        <input type="number" name="three_years_plus_due" value="<?= val('three_years_plus_due', '0') ?>" step="any" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">গত তিন বৎসরের বকেয়া *</label>
                        <input type="number" name="last_three_years_due" value="<?= val('last_three_years_due', '0') ?>" step="any" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">বকেয়ার জরিমানা ও ক্ষতিপূরণ *</label>
                        <input type="number" name="due_interest" value="<?= val('due_interest', '0') ?>" step="any" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">হাল দাবি *</label>
                        <input type="number" name="current_demand" value="<?= val('current_demand', '0') ?>" step="any" required class="form-control-custom">
                    </div>
                </div>
                ...
                <div class="form-grid-4">
                    <div>
                        ...
                        <input type="number" name="total_demand" value="<?= val('total_demand', '0') ?>" step="any" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">মোট আদায় *</label>
                        <input type="number" name="total_collection" value="<?= val('total_collection', '0') ?>" step="any" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">মোট বকেয়া *</label>
                        <input type="number" name="total_due" value="<?= val('total_due', '0') ?>" step="any" required class="form-control-custom">
                    </div>
                    <div>
                        <label class="form-label-custom">মন্তব্য</label>
                        <textarea name="comments" rows="1" class="form-control-custom"><?= val('comments') ?></textarea>
                    </div>
                </div>
                ...
                <div style="margin-top:0.25rem;">
                    ...
                    <div class="section-body" id="ownerSection">
                        <?php
                        $ownersList = $formData['owners'] ?? [];
                        if (empty($ownersList)) {
                            $ownersList = [['name' => '', 'share' => '']];
                        }
                        foreach ($ownersList as $idx => $owner):
                        ?>
                        <div class="row-item" style="display:flex; gap:0.75rem; align-items:center;">
                            <input type="text" name="owners[<?= $idx ?>][name]" value="<?= h($owner['name'] ?? '') ?>" placeholder="মালিকের নাম" <?= $idx === 0 ? 'required' : '' ?> class="form-control" style="flex:1;">
                            <input type="number" step="any" name="owners[<?= $idx ?>][share]" value="<?= h($owner['share'] ?? '') ?>" placeholder="সম্পত্তির পরিমাণ" <?= $idx === 0 ? 'required' : '' ?> class="form-control" style="flex:1;">
                            <?php if ($idx > 0): ?>
                ...
                <div style="margin-top:0.25rem;">
                    ...
                    <div class="section-body" id="dagSection">
                        <?php
                        $dagsList = $formData['dags'] ?? [];
                        if (empty($dagsList)) {
                            $dagsList = [['dag' => '', 'type' => '', 'amount' => '']];
                        }
                        foreach ($dagsList as $idx => $dag):
                        ?>
                        <div class="row-item" style="display:flex; gap:0.75rem; align-items:center;">
                            <input type="text" name="dags[<?= $idx ?>][dag]" value="<?= h($dag['dag'] ?? '') ?>" placeholder="দাগ নম্বর" <?= $idx === 0 ? 'required' : '' ?> class="form-control" style="flex:1;">
                            <input type="text" name="dags[<?= $idx ?>][type]" value="<?= h($dag['type'] ?? '') ?>" placeholder="খতিয়ান শ্রেণি" <?= $idx === 0 ? 'required' : '' ?> class="form-control" style="flex:1;">
                            <input type="text" name="dags[<?= $idx ?>][amount]" value="<?= h($dag['amount'] ?? '') ?>" placeholder="খতিয়ান পরিমাণ" <?= $idx === 0 ? 'required' : '' ?> class="form-control" style="flex:1;">
                            <?php if ($idx > 0): ?>
                                <button type="button" class="btn-remove" onclick="this.closest('.row-item').remove()">×</button>
                            <?php endif; ?>
                        </div>
                ...
                <div>
                    ...
                    <input type="text" name="total_in_words" value="<?= val('total_in_words') ?>" required class="form-control" placeholder="উদা: দশ টাকা মাত্র">
                </div>
                ...
                <div style="display:flex; align-items:center; gap:1rem; margin-top:0.5rem; padding-top:1.25rem; border-top:1.5px solid rgba(102,126,234,0.12);">
                    <a href="index.php" class="btn-back">⬅️ ড্যাশবোর্ডে ফিরে যান</a>
                    <button type="submit" class="btn-submit" id="submitBtn" style="margin-left:auto;">
                        <span id="submitText">💾 আপডেট করুন</span>
                    </button>
                </div>
...
<script>
let ownerIndex = <?= count($ownersList) ?>;
let dagIndex   = <?= count($dagsList) ?>;
